...
class threadpool{
    public:
       ...
       ~threadpool();
       void add_task(T *handler,int mod);
...
void* work(void *thread_pool){
    ...
    while(1){  
        ...
        //从任务队列拿出任务
        tasks<T> temp;
        temp.handler=(pool->task+pool->front)->handler;
        temp.mod=(pool->task+pool->front)->mod;
        ...
        if(temp.mod==0){
            temp.handler->handle_read();
        }else{
            temp.handler->handle_write();
        }
    }
    return nullptr;
}
...
void* produce(void *thread_pool){
    threadpool<T> *pool=(threadpool<T> *)thread_pool; 
     while (1){
        ...
        (pool->task+pool->rear)->handler=temp.handler;
        (pool->task+pool->rear)->mod=temp.mod;
        pool->que.pop();
        pool->task_num++;
...
template<class T>
void threadpool<T>::add_task(T *handler,int mod){
        //加入queue
